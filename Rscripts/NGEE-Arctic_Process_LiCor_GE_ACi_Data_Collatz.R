...
#
#
#  	--- Last updated:  03.30.2017 By Shawn P. Serbin <sserbin@bnl.gov>
####################################################################################################
...
out.dir <- '/Volumes/TEST/Projects/NGEE-Arctic/Data/Barrow/Gas_Exchange/R_Output/Processed_LI6400_Data/NGEE-Arctic_2012-2015_ACi_Data_final.Collatz/'
...
Tau25     <- 2600
Q10Tau    <- 0.57
mm.constants <- list(R=R,Oxygen=Oxygen,Kc25=Kc25,Q10Kc=Q10Kc,Ko25=Ko25,Q10Ko=Q10Ko,Tau25=Tau25,
                     Q10Tau=Q10Tau)
rm(R,Oxygen,Kc25,Q10Kc,Ko25,Q10Ko,Tau25,Q10Tau)
#--------------------------------------------------------------------------------------------------#
...
TleafC  <- ge.data.qc$GE.data[,loc[1]]
ge.data.qc$GE.data$Kc <- mm.constants$Kc25*mm.constants$Q10Kc^((TleafC-25)/10)
ge.data.qc$GE.data$Ko <- mm.constants$Ko25*mm.constants$Q10Ko^((TleafC-25)/10)
ge.data.qc$GE.data$Gstar <- mm.constants$Oxygen/(2*(mm.constants$Tau25*mm.constants$Q10Tau^((TleafC-25)/10)))*1000
#--------------------------------------------------------------------------------------------------#
...
#    print(paste("Jmax: ",round(Jmax[i],2)))
    ...
    if (chk2 >=Vcmax.min.n){
      ...
      params <- data.frame(Vcmax=Vcmax[i],Jmax=-9999,Rd=Rd[i],RMSE.photo=RMSE.photo[i])
      plot.ge.fit(type="A-Ci",sub.data,fit,params,aci.fig.dir,sample.name,f.model=2)
      #rm(f.model)
    }
    rm(fit)
...
output.data <- data.frame(samples,out.diagnostics,Meas.Photo.atCO2R.400,Meas.CiCa.atCO2R.400,
                          ...
                          Vcmax=Vcmax,Rd=Rd,RMSE.DEoptim=RMSE.DEoptim,
                          RMSE.photo=RMSE.photo)
output.dataset <- paste(strsplit(dataset,".csv"),".collatz.processed.csv",sep="")
